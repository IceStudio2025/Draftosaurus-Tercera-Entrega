...
class PlacementRepository
{
    ...
    public function getPlacementsByPlayer(int $gameId, int $playerSeat): array
    {
        try {
            ...
            $query = "SELECT p.placement_id, p.enclosures_id, p.dino_id, p.player_seat,
            ...
            error_log("getPlacementsByPlayer: gameId=$gameId, playerSeat=$playerSeat");
            
            $stmt = $this->conn->prepare($query);
            if (!$stmt) {
                error_log("getPlacementsByPlayer: Error preparando query: " . $this->conn->error);
                return [];
            }
            ...
            error_log("getPlacementsByPlayer: Encontrados " . count($placements) . " placements para playerSeat=$playerSeat");
    ...
    /**
     * Verifica si un recinto tiene colocaciones para un jugador específico
     * RF38: Para validar "Recinto Vacío" - cada jugador tiene su propio tablero
     */
    public function hasPlacementsInEnclosureForPlayer(int $gameId, int $playerSeat, int $enclosureId): bool {
        $query = "SELECT COUNT(*) as count 
                 FROM placement 
                 WHERE game_id = ? AND player_seat = ? AND enclosures_id = ?";

        $stmt = $this->conn->prepare($query);
        if (!$stmt) return false;

        $stmt->bind_param("iii", $gameId, $playerSeat, $enclosureId);
        $stmt->execute();
    ...
    /**
     * Verifica si hay una especie específica en un recinto por código
     * @param int $gameId ID del juego
     * @param int $enclosureId ID del recinto
     * @param string $speciesCode Código de la especie a buscar (ej: 'rojo')
     * @return bool true si existe la especie en el recinto
     */
    public function hasSpeciesInEnclosureByCode(int $gameId, int $enclosureId, string $speciesCode): bool {
        $query = "SELECT COUNT(*) as count 
                 FROM placement p
                 JOIN bag_contents bc ON p.dino_id = bc.bag_content_id
                 JOIN species s ON bc.species_id = s.species_id
        ...
        $stmt->bind_param("iis", $gameId, $enclosureId, $speciesCode);
        $stmt->execute();
    ...
    /**
     * Verifica si hay una especie específica en un recinto para un jugador específico
     * RF39: Para validar "Recinto sin T-REX" - cada jugador tiene su propio tablero
     * @param int $gameId ID del juego
     * @param int $playerSeat Asiento del jugador
     * @param int $enclosureId ID del recinto
     * @param string $speciesName Nombre de la especie a buscar
     * @return bool true si existe la especie en el recinto del jugador
     */
    public function hasSpeciesInEnclosureForPlayer(int $gameId, int $playerSeat, int $enclosureId, string $speciesName): bool {
        $query = "SELECT COUNT(*) as count 
                 FROM placement p
                 JOIN bag_contents bc ON p.dino_id = bc.bag_content_id
                 JOIN species s ON bc.species_id = s.species_id
        ...
        $stmt->bind_param("iiis", $gameId, $playerSeat, $enclosureId, $speciesName);
        $stmt->execute();
    ...
    /**
     * Verifica si hay una especie específica en un recinto para un jugador específico por código
     * RF39: Para validar "Recinto sin T-REX" - cada jugador tiene su propio tablero
     * @param int $gameId ID del juego
     * @param int $playerSeat Asiento del jugador
     * @param int $enclosureId ID del recinto
     ...
     * @return bool true si existe la especie en el recinto del jugador
     */
    public function hasSpeciesInEnclosureByCodeForPlayer(int $gameId, int $playerSeat, int $enclosureId, string $speciesCode): bool {
        $query = "SELECT COUNT(*) as count 
                 FROM placement p
                 JOIN bag_contents bc ON p.dino_id = bc.bag_content_id
                 JOIN species s ON bc.species_id = s.species_id
        ...
        $stmt->bind_param("iiis", $gameId, $playerSeat, $enclosureId, $speciesCode);
        $stmt->execute();
