...
class PlacementDieRollRepository
{
    ...
    /**
     * Obtener la penúltima tirada de la partida (el dado anterior)
     * Útil cuando el jugador que tiró el último dado debe cumplir la restricción del anterior
     */
    public function getPreviousGameRoll(int $gameId): ?array
    {
        $stmt = $this->conn->prepare(
            "SELECT roll_id, die_face, affected_player_seat, created_at 
             FROM placement_die_rolls 
    ...
    /**
     * Eliminar todas las tiradas de dado de un juego
     * Se usa cuando todos los jugadores han colocado y se resetea el dado
     */
    public function clearDieRollsForGame(int $gameId): bool
    {
        $stmt = $this->conn->prepare(
            "DELETE FROM placement_die_rolls WHERE game_id = ?"
        );
